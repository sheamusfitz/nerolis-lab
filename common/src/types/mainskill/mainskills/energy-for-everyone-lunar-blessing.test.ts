...
describe('EnergyForEveryoneLunarBlessing', () => {
  ...
  it('should have correct basic properties', () => {
    expect(EnergyForEveryoneLunarBlessing.name).toBe('Lunar Blessing (Energy For Everyone)');
    expect(EnergyForEveryoneLunarBlessing.description(1)).toBe(
  ...
  it('should calculate correct energy amounts', () => {
    expect(EnergyForEveryoneLunarBlessing.activations.energy.amount(1)).toBe(3);
    expect(EnergyForEveryoneLunarBlessing.activations.energy.amount(3)).toBe(5);
    expect(EnergyForEveryoneLunarBlessing.activations.energy.amount(6)).toBe(11);
  });

  it('should calculate correct self berry amounts with unique count', () => {
    expect(EnergyForEveryoneLunarBlessing.activations.selfBerries.amount(1, 1)).toBe(5);
    expect(EnergyForEveryoneLunarBlessing.activations.selfBerries.amount(6, 3)).toBe(30);
    expect(EnergyForEveryoneLunarBlessing.activations.selfBerries.amount(6, 5)).toBe(32);
  });

  it('should calculate correct team berry amounts with unique count', () => {
    expect(EnergyForEveryoneLunarBlessing.activations.teamBerries.amount(1, 1)).toBe(1);
    expect(EnergyForEveryoneLunarBlessing.activations.teamBerries.amount(6, 3)).toBe(4);
    expect(EnergyForEveryoneLunarBlessing.activations.teamBerries.amount(6, 5)).toBe(9);
  });
